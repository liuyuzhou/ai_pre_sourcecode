...
from chapter9.common.sqlalchemy_conn import db_conn
from concurrent.futures import ThreadPoolExecutor, ProcessPoolExecutor
from chapter9.common.music_model import Song
from chapter9.config import ROOT_PATH

download_path = os.path.join(ROOT_PATH, 'music_file')
...
def download_music(song_mid, song_name):
    try:
        ...
        vkey_url = 'https://c.y.qq.com/base/fcgi-bin/fcg_music_express_mobile3.fcg?' \
                   'loginUin=0&hostUin=0&cid=205361747&uin=0&songmid={}' \
                   '&filename={}.m4a&guid=0'.format(song_mid, file_name)
        vkey_response = session.get(vkey_url, headers=headers)
        vkey = vkey_response.json()['data']['items'][0]['vkey']
        # 下载歌曲
        url = 'http://dl.stream.qqmusic.qq.com/{}.m4a?vkey={}&guid=0&uin=0&fromtag=66'.\
            format(file_name, vkey)
        response = session.get(url, headers=headers)
        music_download_path = download_path + '/{}.m4a'.format(song_name)
...
def get_singer_songs(singer_mid):
    ...
    url = 'https://c.y.qq.com/v8/fcg-bin/fcg_v8_singer_track_cp.fcg?' \
          'loginUin=0&hostUin=0&singermid={}' \
    ...
    for page_num in range(page_count):
        ...
        for i in music_data:
            ...
            song_interval = int(i['musicData']['interval'])
            song_songmid = i['musicData']['songmid']
            # 下载歌曲
            download_music(song_songmid, song_name)
            # 入库处理
            song_obj = Song(song_name=song_name, song_ablum=song_ablum,
                            song_interval=song_interval, song_songmid=song_songmid,
                            song_singer=song_singer, create_time=datetime.datetime.now())
            session_db = db_conn()
...
def get_alphabet_singer(alphabet, page_list):
    for page_num in page_list:
        url = 'https://c.y.qq.com/v8/fcg-bin/v8.fcg?channel=singer&page=list&key=all_all_{}' \
              '&pagesize=100&pagenum={}&loginUin=0&hostUin=0&format=jsonp'.\
            format(alphabet, page_num + 1)
...
def multi_threading(alphabet):
    ...
    url = 'https://c.y.qq.com/v8/fcg-bin/v8.fcg?channel=singer&page=list&' \
          'key=all_all_{}&pagesize=100&pagenum={}&loginUin=0&hostUin=0&format=jsonp'.\
        format(alphabet, 1)
    response = session.get(url, headers=headers)
